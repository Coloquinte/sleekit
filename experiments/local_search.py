...
for root in it:
    ...
    ls10_weight = quantize_with_scaling(
        weight, sc, cb, H=hessian, damp=args.damp, nb_ls_moves=10
    )
    ls10_error = quantization_error(weight, ls10_weight, H=hessian)
    ls100_weight = quantize_with_scaling(
        weight, sc, cb, H=hessian, damp=args.damp, nb_ls_moves=100
    )
